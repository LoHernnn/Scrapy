...
def _get_log():
    global _log
    if _log is None:
        _log = Logger.get_logger("Database")
    return _log
...
def create_table_listing(cur, conn):
    cur.execute("""
...
def create_table_crypto_ranks(cur, conn):
    cur.execute("""
...
def create_table_base(cur, conn):
    cur.execute("""
...
def create_table_detail(cur, conn):
    cur.execute("""
...
def create_table_data_binance(cur, conn):
    cur.execute("""
...
def insert_crypto(cur,conn, name, symbol, id_coingecko, symbol_binance):
    cur.execute("""
...
def insert_or_update_rank(cur, conn, crypto_id, rank):
    try:
        cur.execute("""
        ...
        conn.rollback()
        _get_log().error(f" Erreur lors de l'insertion/maj du rank (crypto_id={crypto_id}): {e}")

def insert_cyptos_base(cur, conn, crypto_id, data):
    try:
        cur.execute("""
        ...
        conn.rollback()
        _get_log().error(f" Erreur lors de l'insertion dans cyptos_data_base (crypto_id={crypto_id}): {e}")


def insert_cyptos_data_details(cur, conn, crypto_id, data):
    try:
        ...
        conn.rollback()
        _get_log().error(f" Erreur lors de l'insertion dans cyptos_data_details (crypto_id={crypto_id}): {e}")



def insert_cyptos_data_binance(cur,conn, crypto_id, data):
    try:
        cur.execute("""
        ...
        conn.rollback()
        _get_log().error(f" Erreur lors de l'insertion dans cyptos_data_binance (crypto_id={crypto_id}): {e}")
...
def select_latest_data(cur, conn, verbose: bool = True) -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        cur.execute("SELECT MAX(timestamp) FROM cyptos_data_base;")
        latest_base_ts = cur.fetchone()[0]
        if latest_base_ts:
            cur.execute(
                """
            ...
            rows = cur.fetchall()
            result["base"] = _rows_to_dicts(cur, rows)

        cur.execute("SELECT MAX(timestamp) FROM cyptos_data_details;")
        latest_details_ts = cur.fetchone()[0]
        if latest_details_ts:
            ...
            rows = cur.fetchall()
            result["details"] = _rows_to_dicts(cur, rows)

        cur.execute("SELECT MAX(timestamp) FROM cyptos_data_binance;")
        latest_binance_ts = cur.fetchone()[0]
        if latest_binance_ts:
            ...
            rows = cur.fetchall()
            result["binance"] = _rows_to_dicts(cur, rows)
